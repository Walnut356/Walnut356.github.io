...
The source code is available [here](https://github.com/Walnut356/twovec), and the crate listing here
...
Standard library types are a *generalist* solution that must be correct and useful in almost all cases. "Almost all" is not "every". There exists alternative ways of expressing your intent to the computer. In the same way that performance tuning is about informing the compiler about as many invariants as possible, sometimes it's okay to have a slightly gross data structure if it means it reflects your intentions better. In some cases, this also reduces the complexity of the final algorithm. Rules like "one type per array" or even "all array elements must be the same size" is just the tip of the iceberg. Calling them "rules" is itself sortof wrong; "guideline" seems more accurate. It's easy to forget that when reading discourse and advice about programming.
